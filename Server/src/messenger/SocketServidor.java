...
public class SocketServidor implements Runnable {
            ...
            if(msg.tipo.equals("iniciar sesión")){
                if(encontrarHiloDeUsuario(msg.de) == null){
                    if(db.checarInicio(msg.de, msg.contenido)){
                        clients[encontrarCliente(ID)].usuario = msg.de;
                        clients[encontrarCliente(ID)].enviar(new Mensaje("iniciar sesión", "servidor", "verdadero", msg.de));
                        anuncio("nuevo usuario", "servedior", msg.de);
                        mandarListaDeUsuarios(msg.de);
                    }
                    else{
                        clients[encontrarCliente(ID)].enviar(new Mensaje("iniciar sesión", "servidor", "salso", msg.de));
                    } 
                }
                else{
                    clients[encontrarCliente(ID)].enviar(new Mensaje("iniciar sesión", "servidor", "falso", msg.de));
                }
            }
                ...
                if(msg.para.equals("todos")){
                    ...
                    clients[encontrarCliente(ID)].enviar(new Mensaje(msg.tipo, msg.de, msg.contenido, msg.para));
                }
            }
            else if(msg.tipo.equals("test")){
                clients[encontrarCliente(ID)].enviar(new Mensaje("test", "servidor", "OK", msg.de));
            }
                ...
                if(encontrarHiloDeUsuario(msg.de) == null){
                    if(!db.salidaUsuario(msg.de)){
                        ...
                        clients[encontrarCliente(ID)].usuario = msg.de;
                        clients[encontrarCliente(ID)].enviar(new Mensaje("Resgistrarse", "servidor", "verdadero", msg.de));
                        clients[encontrarCliente(ID)].enviar(new Mensaje("iniciar sesión", "servidor", "verdadero", msg.de));
                        anuncio("newuser", "SERVER", msg.de);
                        mandarListaDeUsuarios(msg.de);
                    }
                    else{
                        clients[encontrarCliente(ID)].enviar(new Mensaje("Registrarse", "servidor", "falso", msg.de));
                    }
                }
                else{
                    clients[encontrarCliente(ID)].enviar(new Mensaje("Registrarse", "servidor", "FALSE", msg.de));
                }
            }
            else if(msg.tipo.equals("subir_req")){
                if(msg.para.equals("todos")){
                    clients[encontrarCliente(ID)].enviar(new Mensaje("mensaje", "servidor", "Subiendo para 'todo' esta prohibido", msg.de));
                }
                else{
    ...
    public void mandarListaDeUsuarios(String toWhom){
        for(int i = 0; i < clienteConteo; i++){
            encontrarHiloDeUsuario(toWhom).enviar(new Mensaje("nuevo usuario", "sevidor", clients[i].usuario, toWhom));
        }
    }
    
    public servidorHilo encontrarHiloDeUsuario(String usr){
        for(int i = 0; i < clienteConteo; i++){
            if(clients[i].usuario.equals(usr)){
                return clients[i];
            }
        }
        return null;
    ...
    int pos = encontrarCliente(ID);
        if (pos >= 0){  
            servidorHilo toTerminate = clients[pos];
	...
	if (clienteConteo < clients.length){  
            ui.jTextArea1.append("\nCliente acceptado: " + socket);
	    clients[clienteConteo] = new servidorHilo(this, socket);
	    try{  
	      	clients[clienteConteo].open(); 
	        clients[clienteConteo].start();  
	        clienteConteo++; 
	    }
	    catch(IOException ioe){  
            ...
            ui.jTextArea1.append("\nCliente rechazado: maximo " + clients.length + " alcanzado.");
	}
    }
}
